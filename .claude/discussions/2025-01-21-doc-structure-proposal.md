...
| 🚫 `SUPERSEDED` | Заменена новой дискуссией |
...
| Поле | Значение |
|------|----------|
| **Статус** | 🔵 ACTIVE / 🟡 DEVELOPING / ⚪ DEPRECATED |
...
> Сервис в статусе 🟡 DEVELOPING. Архитектура будет заполнена после завершения первого ADR.
...
| Статус | Emoji | Описание |
|--------|-------|----------|
| `DEVELOPING` | 🟡 | Сервис в разработке (первый ADR не DONE) |
| `ACTIVE` | 🔵 | Сервис активен (есть хотя бы один ADR DONE) |
| `DEPRECATED` | ⚪ | Сервис устарел, планируется удаление |

**Переходы:**
- `DEVELOPING → ACTIVE`: первый ADR → DONE
- `ACTIVE → DEPRECATED`: создан ADR на удаление/замену сервиса

**Примечание:** Эти статусы относятся к сервису в целом, не к документам.
...
/{type}-update <id>                  # Обновить содержимое (редактирование)
```
