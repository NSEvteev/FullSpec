...
|--------|-------------------|----------|
| [1. Обзор](#1-обзор-readme) | `specs/README.md` | Структура /specs, связь с /doc/, принятые решения |
| [2. Статусы](#2-статусы-statusesmd) | `specs/statuses.md` | Унифицированная система статусов (без чек-листов) |
| [3. Workflow](#3-workflow-workflowmd) | `specs/workflow.md` | Полный workflow с возвратами |
| [4. Discussions](#4-discussions-discussionsmd) | `specs/discussions.md` | Формат, статусы, **чек-листы переходов** |
| [5. Impact](#5-impact-impactmd) | `specs/impact.md` | Формат, **чек-листы**, создание сервисов |
| [6. ADR](#6-adr-adrmd) | `specs/adr.md` | Формат, **чек-листы**, breaking changes, конфликты |
| [7. Plans](#7-plans-plansmd) | `specs/plans.md` | Формат, **чек-листы**, GitHub Issues, версионирование |
| [8. Architecture](#8-architecture-architecturemd) | `specs/architecture.md` | Формат (без статусов, живой документ) |
| [9. Glossary](#9-glossary-glossarymd) | `specs/glossary.md` | Формат, workflow глоссария (без статусов) |
...
## ~~2.9. Чек-листы переходов~~ (перенесены)

> **Чек-листы переходов перенесены в инструкции соответствующих типов документов:**
> - Discussion → [4.6. Чек-листы переходов Discussion](#46-чек-листы-переходов-discussion)
> - Impact → [5.8. Чек-листы переходов Impact](#58-чек-листы-переходов-impact)
> - ADR → [6.10. Чек-листы переходов ADR](#610-чек-листы-переходов-adr)
> - Plan → [7.10. Чек-листы переходов Plan](#710-чек-листы-переходов-plan)
...
Шаблон и пример заполнения находятся в файле шаблона. Основные секции:

- **Метаданные** — статус, область, дата, автор
- **Контекст** — проблема, область обсуждения, вне области
- **Варианты решения** — таблица + детальное описание каждого
- **Критерии оценки** — с весами (Высокий/Средний/Низкий)
- **Сравнение вариантов** — звёздочная шкала ⭐⭐⭐
- **Вопросы и допущения** — для уточнения
- **Решение** — заполняется после APPROVED
- **Ревью решения** — что сможете/не сможете, риски
- **Связанные документы** — Impact, ADR
...
- **Метаданные** — статус, ссылка на Discussion, дата
- **Область анализа** — что анализируем, вне области
- **Затронутые сервисы** — таблица с типом изменений и ссылками на ADR
- **Бизнес-логика** — описание бизнес-процессов
- **Технический анализ** — по каждому сервису: компоненты, БД, зависимости
- **Риски и зависимости** — таблица с митигацией
...
При создании нового сервиса скилл `/spec-create adr` автоматически создаёт структуру:
...
- **Метаданные** — статус, Impact, сервис, Breaking Changes, дата
- **Контекст** — проблема, требования
- **Решение** — описание + краткая суть
- **Обоснование** — почему выбрали, преимущества, недостатки
- **Рассмотренные альтернативы** — с причинами отклонения
- **Бизнес-логика** — таблица проверки (описано в Impact?)
- **Влияние на архитектуру** — текущее → планируемое
- **Задачи для плана** — чеклист
- **Риски** — таблица с митигацией
- **Последствия** — позитивные/негативные
- **Breaking Changes** — опционально, с планом миграции
- **Заменяет** — опционально, ссылка на старый ADR
- **План реализации** — ссылка после создания Plan
...
- **Метаданные** — статус, ADR, сервис, дата
- **Задачи** — по фазам (БД, Backend, Тесты, Документация)
- **Зависимости** — таблица (что зависит от чего)
- **Оценка** — сложность по фазам (Low/Medium/High)
- **GitHub Issues** — таблица со ссылками и статусами
- **Критерий готовности** — условия для DONE
...
- **Принципы и ограничения** — Stateless, Single Responsibility, Fail Fast; compliance, производительность, доступность
- **Текущее состояние** — компоненты со ссылками на ADR
- **Зависимости** — таблица (компонент → зависимость → ADR)
- **Связи с другими сервисами** — upstream/downstream
- **Диаграммы** — ASCII-схемы компонентов
- **История изменений** — версия, ADR, дата, автор, что изменилось
...
```
/spec-status {service}/plans/ jwt-migration approved
    │
    └── Каскад: ADR → RUNNING, Impact → RUNNING

/spec-status {service}/plans/ jwt-migration done
    │
    └── Каскад: ADR → DONE
