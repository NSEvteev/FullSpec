...
При работе с сервисной документацией ОБЯЗАТЕЛЬНО использовать скиллы:
  - `/service-create` - создание сервисного документа (заглушка при Design → WAITING)
...
Сервисный документ `services/{svc}.md` создаётся в два этапа:
  1. **Заглушка** при Design → WAITING (только Резюме + Planned Changes)
  2. **Full** при ADR → DONE (заполнение секций 1-6 из дельты)

Детекция режима: отсутствие `created-by` в frontmatter = заглушка.
