...
Upward feedback происходит **во время проектирования** — это нормальная часть workflow. Обратный каскад Code → Specs (решение #22) запускается **после разработки**.

#### Параллельные дискуссии: резервации и конфликт-детекция
...
**Механизм A — Резервации в архитектуре:**

Когда Design переходит в APPROVED, в затронутых файлах `architecture/` добавляется секция:
...
LLM при чтении AS IS **обязан** учитывать Planned Changes. Секция удаляется при обновлении живого документа (ADR → DONE).

**Механизм C — Конфликт-детекция:**

При создании Impact для новой дискуссии LLM проверяет:
1. Есть ли RUNNING дискуссии, затрагивающие те же сервисы?
2. Есть ли Planned Changes в `architecture/` для затронутых сервисов?

Если да — обязательный маркер:

```markdown
[КОНФЛИКТ С: discussions/001-oauth2-authorization.md]
Затронутые сервисы пересекаются: auth-service, gateway-service.
Planned Changes: API endpoints, data model.
Необходимо учесть при проектировании.
```

Пользователь решает: продолжать параллельно (с учётом) или подождать завершения первой дискуссии.
...
| `architecture/services/{svc}.md` | Архитектура сервиса. Planned Changes для параллельных дискуссий | ADR → DONE (+ Planned Changes при Design → APPROVED) |
...
| 22 | Обратная связь Code → Specs | **Обратный каскад снизу вверх** (черновое). Plan → Test Spec → ADR → Design → Impact → Discussion. На каждом уровне: затрагивает? Если нет — стоп |
...
| 30 | Параллельные дискуссии | **Резервации** (Planned Changes в architecture/) + **конфликт-детекция** (маркер `[КОНФЛИКТ С: ...]`) |
...
| # | Вопрос | Статус |
|---|--------|--------|
| 1 | Обратный каскад Code → Specs (решение #22) | Черновое решение принято. Требуется детальная проработка: правила стопа, формат обновления документов, автоматизация |
| 2 | Описания объектов внутри src/ | Нужно ли документировать объекты внутри `/src/{svc}/`? Где хранить (docstrings / отдельные файлы)? Визуализировать ли на диаграммах? Как синхронизировать с кодом? |
| 3 | Параллельные дискуссии | **Решено** (решение #30) |
| 4 | Статусы жизненного цикла | Предложена модель: DRAFT → AGREED → WAITING → RUNNING → DONE + CONFLICT. Обратная связь Code→Specs с уровнями серьёзности. Каскадный перевод в RUNNING при утверждении всех планов. Требуется детальная проработка |
