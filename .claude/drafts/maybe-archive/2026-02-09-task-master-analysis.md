...
| Зависимости | Числовые ID с валидацией | Ссылки на тест-спеки |
| Тест-стратегия | Поле `testStrategy` | Отдельный уровень Test Spec |
| Контекст | PRD → задачи (без спецификаций) | Discussion → ... → Test Spec → Plan |
...
| **testStrategy на задаче** | Ссылки на конкретные сценарии из Test Spec | Поле задачи в Plan |
...
| PRD-парсинг | У нас 5 уровней спецификаций до Plan | Discussion → ... → Test Spec |
...
| # | Вопрос | Решение |
|---|--------|---------|
| 1 | Формат задач в Plan | **Структурированные блоки.** Каждая задача = заголовок + метаданные (сложность, приоритет, зависимости, тест-спек, дельта) + чек-лист подзадач |
...
| 3 | Нужен ли отдельный шаг analyze-complexity | **Нет.** LLM оценивает сложность при генерации Plan в одном проходе. В Task Master отдельный шаг нужен потому, что PRD → задачи идёт без контекста. У нас контекст есть (Discussion → ... → Test Spec) |
