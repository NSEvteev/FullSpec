...
- [Содержание](#содержание)
  ...
  - [3. Воркфлоу финальной валидации (create-test.md)](#3-воркфлоу-финальной-валидации-create-testmd)
...
### 3. Воркфлоу финальной валидации (create-test.md)

**Назначение:** SSOT-инструкция оркестрации финальной валидации. Скилл `/test` — тонкий указатель на этот воркфлоу (по шаблону SKILL.md → SSOT).

**SSOT-цепочка:**
```
/test (SKILL.md — указатель) → create-test.md (этот воркфлоу) → validation-development.md (чек-лист проверок)
                                                               → standard-docker.md § 8 (тестовое окружение)
                                                               → standard-testing-system.md (паттерны тестов)
```

**Шаги воркфлоу (обновлено после верификации — пробелы C1-C4):**
...
- `/test` — **скилл** (тонкий указатель на create-test.md, ~40 строк по шаблону SKILL.md)
...
TASK 2: Создать SSOT-инструкцию create-test.md
  description: >
    Драфт: секция "3. Воркфлоу финальной валидации (create-test.md)" + секция "6.3" | Пробелы: C1-C4
    Скилл: /instruction-create
...
TASK 3: Создать скилл /test
  ...
  description: >
    Скилл: /skill-create
    Создать .claude/skills/test/SKILL.md — тонкий указатель на create-test.md.
    Шаблон: как merge/SKILL.md — SSOT-ссылка + "Выполнить шаги из SSOT".
