...
Создание инструкций через `/instruction-create` с валидацией meta-reviewer.

**Требования к агентам (meta-agent):**
...
- **В процессе создания** — вызвать meta-reviewer для семантического анализа

**Интеграция с meta-reviewer:**
meta-agent ДОЛЖЕН вызвать meta-reviewer через Task tool со следующим промптом:
```
...
```
meta-agent исправляет P1-проблемы по рекомендациям meta-reviewer до завершения задачи.
...
**Статус: ВЫПОЛНЕНО** — все стандарты созданы и прошли семантический анализ meta-reviewer (P1/P2 исправления применены)
