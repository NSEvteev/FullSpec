...
> **СТАТУС ПЕРЕНОСА: ЗАВЕРШЁН (99%)**
>
> Контент этого черновика перенесён в 5 целевых документов:
...
> - "Каждый standard-*.md содержит" (§ 9) — мета-информация о формате инструкций
> - "Порядок создания инструкций" (§ 9) — операционный план, покрыт drafts/2026-02-10-specs-documents-plan.md
...
> **ℹ️ Мета-секция черновика.** Не подлежит переносу — описывает контекст создания.
...
> **✅ Перенесено** → [Навигатор SDD § 1](../../../specs/.instructions/standard-specs-workflow.md#1-философия) — все 4 принципа, полностью
...
> **✅ Перенесено:**
> - Mermaid-диаграмма 6 уровней, таблица объектов, Дискуссия-контейнер, Расширяемость → [Навигатор SDD § 2](../../../specs/.instructions/standard-specs-workflow.md#2-шесть-уровней)
> - Таблица живых документов (Архитектура, Тесты, Глоссарий) → [Справочник SDD § 5](../../../specs/.instructions/standard-specs-reference.md#5-живые-документы)
...
| **ADR** | КАК КОНКРЕТНО | `specs/services/{svc}/adr/` | Какое техническое решение для сервиса? Что меняется (ADDED/MODIFIED/REMOVED)? | Дизайн → План тестов(и) |
| **План тестов** | КАК ПРОВЕРЯЕМ | `specs/services/{svc}/plan-test/` | Как проверяем решение? Какие тестовые сценарии? | ADR → План (1:1) |
...
> **✅ Перенесено:**
> - 7 статусов, диаграмма, таблица переходов → [Справочник SDD § 2](../../../specs/.instructions/standard-specs-reference.md#2-статусы)
> - Откат артефактов (ROLLING_BACK) → [Справочник SDD § 3.4](../../../specs/.instructions/standard-specs-reference.md#откат-артефактов)
> - Прямой поток DRAFT→WAITING→RUNNING → [Навигатор SDD § 4.2](../../../specs/.instructions/standard-specs-workflow.md#прямой-поток)
> - Каскад RUNNING → [Справочник SDD § 3.1](../../../specs/.instructions/standard-specs-reference.md#каскад-running)
> - Каскадное завершение DONE (диаграмма, таблица, правило, глоссарий) → [Справочник SDD § 3.2](../../../specs/.instructions/standard-specs-reference.md#каскадное-завершение-done)
> - Каскад REJECTED/ROLLING_BACK (правила, 2 примера) → [Справочник SDD § 3.3](../../../specs/.instructions/standard-specs-reference.md#каскад-rejected-и-rolling_back)
> - Обратная связь Code→Specs (фазы 1-2, 3 сценария, кросс-цепочечная) → [Справочник SDD § 4](../../../specs/.instructions/standard-specs-reference.md#4-обратная-связь-code-specs)
> - Связи / frontmatter (YAML-пример, таблица правил, Milestone) → [Справочник SDD § 1](../../../specs/.instructions/standard-specs-reference.md#1-связи-и-frontmatter)
...
ADR auth отклоняется:
  ...
  ├── вниз: План тестов auth (RUNNING) → ROLLING_BACK → REJECTED
...
ADR (RUNNING) → ROLLING_BACK → REJECTED
  ├── вниз: План тестов (RUNNING) → ROLLING_BACK → REJECTED
...
| **Флаг** (контракты между пакетами) | Plan / План тестов | Рабочие правки — LLM автономно обновляет документы, продолжает работу и выводит в чат краткое резюме изменений (не дожидаясь ответа). Статус не меняется. Фаза 2 не нужна |
| **CONFLICT** (API сервиса, data model, пакеты) | ADR или выше | Все затронутые документы + их дочерние → **CONFLICT**. Работы останавливаются. Проверка продолжается вверх до первого незатронутого → **СТОП**. **Исключение:** незатронутый План тестов не означает, что ADR не затронут — проверять ADR всегда |
...
Design: "auth отвечает за токены, gateway — за rate limiting, users — за профили"
  ...
  (+ План тестов и Plan для каждого)
```
...
Результат:
  ADR auth, План тестов auth, Plan auth → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  2. План тестов auth пересматривается
...
Фаза 1 — Обнаружение (↑):
  ...
  Design затронут? → Да (блоки взаимодействия изменились —
    ...
    → Design → CONFLICT → ВСЕ дети:
      ...
      → ВСЕ их дети (План тестов, Plan) → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  3. План тестов для каждого пересматриваются
...
Фаза 1 — Обнаружение (↑):
  ...
  Impact затронут? → Да ("3 сервиса" → "все сервисы проекта")
    → Impact → CONFLICT → Design → CONFLICT
      → ВСЕ ADR, План тестов, Plan → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  4. План тестов для каждого пересматриваются
...
children:
  - services/auth/adr/adr-0001-jwt-to-oauth2.md
  - services/gateway/adr/adr-0001-oauth2-proxy.md
status: WAITING
...
4. **ADR** → parent: Дизайн, children: План тестов(и)
5. **План тестов** → parent: ADR, children: План (1:1)
6. **План** → parent: План тестов, children: нет (терминальный)
...
> **✅ Перенесено:**
> - Полная диаграмма воркфлоу (Пользователь → Разработка) → [Навигатор SDD § 4.1](../../../specs/.instructions/standard-specs-workflow.md#полная-диаграмма-воркфлоу)
> - Общий паттерн объекта (CLARIFY→GENERATE→VALIDATE→REVIEW, диаграмма) → [Навигатор SDD § 4.3](../../../specs/.instructions/standard-specs-workflow.md#общий-паттерн-объекта)
> - Фильтрация Design→ADR (секции, блоки взаимодействия, Greenfield, дельта-блоки) → [Навигатор SDD § 5.1](../../../specs/.instructions/standard-specs-workflow.md#фильтрация-design--adr)
> - Фильтрация ADR→План тестов → [Навигатор SDD § 5.2](../../../specs/.instructions/standard-specs-workflow.md#фильтрация-adr--план тестов)
> - Shared код /shared/ (таблица, пример UserCreatedEvent, 5 правил) → [Навигатор SDD § 5.3](../../../specs/.instructions/standard-specs-workflow.md#shared-код-shared)
> - Upward feedback (таблица, механика, правило остановки) → [Навигатор SDD § 5.4](../../../specs/.instructions/standard-specs-workflow.md#upward-feedback)
> - Параллельные дискуссии (Planned Changes) → [Навигатор SDD § 6](../../../specs/.instructions/standard-specs-workflow.md#6-параллельные-дискуссии)
...
План тестов определяет **что тестировать** (сценарии, данные, ожидаемые результаты). **Как** реализовать тесты — задача Плана.
...
> **✅ Перенесено** → [Навигатор SDD § 3](../../../specs/.instructions/standard-specs-workflow.md#3-зоны-ответственности) — таблица 10 зон + ASCII-диаграмма границ specs/проект
...
> **✅ Перенесено** → [Справочник SDD § 6](../../../specs/.instructions/standard-specs-reference.md#6-clarify-и-блокирующие-правила) — Clarify на каждом уровне (таблица + механизм + пропуск), маркер [ТРЕБУЕТ УТОЧНЕНИЯ] (4 правила + разрешение), Dependency Barrier (2 режима + примеры кода)
...
> **✅ Перенесено** → [standard-plan.md](../../../specs/.instructions/plan-dev/standard-plan.md) — полностью:
> - Версионирование (ADR chain, DONE-ADR, Очистка REJECTED) → § 1
> - Формат задач (шаблон 2 задач, таблица 5 полей, порядок, подзадачи) → § 2
> - Создание Issues (5-шаговая механика, корректировка Milestone) → § 3.1
> - Маппинг Plan→Issues (диаграмма, таблица 7 строк, sub-issues) → § 3.2-3.3
> - CONFLICT и задачи (таблица 4 действий, done-подзадачи) → § 4
...
| **План тестов** | Ссылка на конкретный сценарий из План тестов. Определяет, что именно тестировать |
...
│   ├── design-NNNN-topic.md            #   Секции по сервисам + блоки взаимодействия + системные тест-сценарии
│   └── README.md
│
...
| `tests/system/` | Межсервисные планы тестов (e2e, integration, load) | Design → DONE |
| `tests/services/{svc}/` | Внутрисервисные планы тестов (e2e, integration, unit) | План тестов → DONE |
...
**Проблема:** Между `architecture/services/{svc}.md` (высокоуровневое описание сервиса) и реальным кодом в `src/{svc}/` существует навигационный разрыв. Когда LLM берёт задачу из GitHub Issue, он знает ЧТО делать (Plan), КАК проверить (План тестов), КАКОЕ решение принято (ADR) — но не знает КУДА СМОТРЕТЬ в коде и ЧТО МОЖНО МЕНЯТЬ самостоятельно.

```
Issue → Plan → План тестов → ADR → architecture/services/{svc}.md
                                          ↓
...
Эти три уровня напрямую связаны с механизмом обратной связи Code → Specs (раздел 3): "свободно" = рабочие правки, "флаг" = возможный уровень План тестов, "CONFLICT" = уровень ADR и выше.
...
|--------|--------|--------|
| Дискуссия | `disc-NNNN-topic.md` | `disc-0001-oauth2-authorization.md` |
| Импакт | `impact-NNNN-topic.md` | `impact-0001-oauth2-authorization.md` |
| Дизайн | `design-NNNN-topic.md` | `design-0001-oauth2-service-design.md` |
| ADR | `adr-NNNN-topic.md` | `adr-0001-jwt-to-oauth2.md` |
| План тестов | `plan-test-NNNN-topic.md` | `plan-test-0001-oauth2-tests.md` |
| План разработки | `plan-dev-NNNN-topic.md` | `plan-dev-0001-jwt-migration.md` |
...
> - Таблица стандартов объектов (11 строк с путями) → [Навигатор SDD § 7](../../../specs/.instructions/standard-specs-workflow.md#7-стандарты-объектов)
> - Дерево .instructions/ — аналогичная информация в таблице § 7, само дерево не дублировано
> - **НЕ перенесено:** "Каждый standard-*.md содержит: правила, принципы, шаблон, чек-лист" — мета-информация. Рекомендуется перенести в стандарт объектных инструкций (`standard-instruction.md`) или в описание шаблона
> - **НЕ перенесено:** "Порядок создания инструкций" (11 шагов) — операционный план, перекрыт планом в `drafts/2026-02-10-specs-documents-plan.md`
...
├── plan-test/                  →  загружается при работе с плана тестовми
...
- Принципы, специфичные для этого типа (архитектурные — в ADR, тестовые — в План тестовs)
...
> **✅ Перенесено:**
> - 21 механическое решение (#5, #7-9, #16-17, #22-23, #25-26, #31, #33-34, #38-41, #43-44, #49, #51) → [Справочник SDD § 9](../../../specs/.instructions/standard-specs-reference.md#9-решения)
> - 30 навигационных решений (#1-4, #6, #10-15, #18-21, #24, #27-30, #32, #35-37, #42, #45-48, #50) → [Навигатор SDD § 8](../../../specs/.instructions/standard-specs-workflow.md#8-решения)
> - Все 51 решение распределены без потерь

| # | Вопрос | Решение |
|---|--------|---------|
| 1 | Naming входного объекта | **Дискуссия** — гибкий контейнер для фич, user stories, предложений, требований |
| 2 | Уровни иерархии | **6 уровней** (Discussion → Impact → Design → ADR → План тестов → Plan), расширяемо |
...
| 15 | Тестовые спецификации | **Два типа.** Системные — секция внутри Design, при DONE → `specs/tests/system/`. Сервисные — формальный уровень 5 (`specs/services/{svc}/plan-test/`), при DONE → `specs/tests/services/{svc}/` |
...
| 22 | Обратная связь Code → Specs | **Последовательная проверка снизу вверх.** Plan/План тестов — рабочие правки. ADR и выше — CONFLICT + каскад CONFLICT на всех детей. СТОП на первом незатронутом. **Исключение:** незатронутый План тестов не означает, что ADR не затронут — ADR проверять всегда |
...
| 29 | Уровень План тестов (ATDD) | **Уровень 5** между ADR и Plan. План тестов определяет сценарии ДО плана реализации |
...
| 39 | Рабочие правки Plan/План тестов | **Автономно.** LLM обновляет документы, продолжает работу, информирует пользователя. Подтверждение не требуется |
...
| 48 | Task management в Plan | **Структурированные блоки** (не чек-лист). Каждая задача: сложность (1-10), приоритет, зависимости, ссылка на план тестов, дельта из ADR, подзадачи (чек-лист). Сложность оценивается LLM в одном проходе (отдельный шаг не нужен). Порядок задач = порядок выполнения. scope-adjustment покрыт CONFLICT-механизмом. Задача → Issue, подзадачи → чек-лист в body Issue. Интеграция с существующим [standard-issue.md](/.github/.instructions/issues/standard-issue.md) |
| 49 | Статус ROLLING_BACK | **Добавлен.** RUNNING/DONE → ROLLING_BACK → REJECTED. Откат артефактов по уровням: Plan (Issues, PR), План тестов (specs/tests/), ADR (architecture/services/, tech standards), Design (architecture/system/, domains/, tests/system/). Impact/Discussion — no-op для единообразия. WAITING → DRAFT добавлен для кросс-цепочечной обратной связи |
...
> **✅ Все решены.** Контент не требует переноса — информационная секция черновика.
