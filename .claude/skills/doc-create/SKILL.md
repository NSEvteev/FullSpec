...
### Какие файлы нельзя документировать?

| Путь | Причина |
|------|---------|
| `/doc/**` | Уже документация |
| `/.claude/**` | Содержит собственную документацию (SKILL.md, инструкции) |
| `/.git/**` | Служебные файлы Git |
| `/tests/**` | Тесты (описываются в /.claude/instructions/tests/) |
| `*.md`, `*.rst`, `*.txt` | Самоописывающиеся файлы |

### Как документировать файлы в /.claude/?

Файлы в `/.claude/` имеют собственную документацию:
- Скиллы → `SKILL.md`
- Инструкции → сами являются документацией

Используйте `/skill-update` или `/instruction-update` для изменения.

### Что делать, если документация уже существует?

Скилл предложит три варианта:
1. **Перезаписать** — создать заново
2. **Редактировать** — вызвать `/doc-update`
3. **Отменить**

### Почему не извлекаются функции?

**Возможные причины:**
- Файл содержит только константы/типы
- Нестандартный синтаксис
- Язык не поддерживается

В этом случае создаётся минимальный шаблон для ручного заполнения.

### Можно ли документировать бинарные файлы?

Нет. Скилл работает только с текстовыми файлами, из которых можно извлечь API.

### Как отменить созданную документацию?
