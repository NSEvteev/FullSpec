...
| Файл | IN | OUT |
|------|-----|-----|
| `project.md` | Дерево проекта, зоны папок | Содержимое файлов |
| `instructions.md` | Зеркальная структура, зоны инструкций | Содержимое инструкций |
| `mapping.md` | Связь инструкций с папками, диаграмма | Детали реализации |
...
| Папка | IN | OUT |
|-------|-----|-----|
| `github/` | actions, workflows, templates, CODEOWNERS | Git правила → meta/git/ |
...
| Папка | IN | OUT |
|-------|-----|-----|
| `meta/git/` | commits, branches, review, merge | GitHub Actions → github/ |
| `meta/instructions/` | types, validation, workflow, relations | Содержимое инструкций |
| `meta/links/` | format, patterns, validation | Конкретные ссылки в файлах |
| `meta/skills/` | rules, parameters, errors, state | Код скиллов → /.claude/skills/ |
| `meta/agents/` | structure, prompts, tools | Код агентов → /.claude/agents/ |
| `meta/scripts/` | naming, structure, hooks | Код скриптов → /.claude/scripts/ |
| `meta/state/` | format, lifecycle, cleanup | Файлы состояний |
| `meta/drafts/` | naming, lifecycle, cleanup | Файлы черновиков |
