...
- Добавление/изменение секций SVC-N (новый номер = max + 1) — включая подсекции (Компоненты, Данные, API)
- Добавление/изменение компонентов (CMP-N) внутри секции SVC
- Добавление/изменение данных (DATA-N) внутри секции SVC
- Добавление/изменение API (API-N) внутри секции SVC
...
4. Добавить секцию: ## SVC-4: notification-service (Тип: Новый, Уверенность: Предположительно)
   с описанием флоу (1–2 абзаца) и 3 подсекциями (Компоненты, Данные, API — заглушки)
