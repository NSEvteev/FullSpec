...
Использовать Grep для поиска `{old}.md` во всех `.md` файлах, затем заменить.

### При изменении заголовков

Если заголовки (якоря) в `services/{svc}.md` изменились — проверить все внешние ссылки на эти якоря.
