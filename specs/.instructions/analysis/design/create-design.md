...
3. **Генерация** — заполнение Резюме + секция "Выбор технологий" (7 критериев, AskUserQuestion пачками по 4) + заголовки SVC-N (h2, без подсекций)
...
**Результат:** partial design.md (Резюме + Выбор технологий с "Выбрано" + заголовки SVC-N).
...
4. Обработка предложений (оркестратор, НЕ агент):
   ...
   - Принятый PROP: применить изменения к документу, удалить PROP из секции «Предложения»
   - Отклонённый PROP: перенести в секцию «Отвергнутые предложения» с причиной отклонения
   - После обработки всех PROP: если секция «Предложения» пуста — удалить секцию
...
| 6 | Per-tech стандарты | Через `/technology-create` для каждой технологии, отмеченной как **"Выбрано"** в секции "Выбор технологий". Если per-tech стандарт уже существует — пропустить |
...
- [ ] Резюме заполнено
- [ ] Секция "Выбор технологий" заполнена с "Выбрано"
- [ ] Заголовки SVC-N добавлены (h2, без подсекций)
