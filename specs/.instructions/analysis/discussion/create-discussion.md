...
| Ответ | Действие |
|-------|----------|
| Запустить ревью агентом (Recommended) | Запустить агента [discussion-reviewer](/.claude/agents/discussion-reviewer/AGENT.md) с путём к документу. Агент запишет PROP-N в секцию "Предложения". После возврата агента — обработать PROP-N (шаг 6, подраздел "Генерация и проработка предложений") → вернуться к шагу 8 |
