...
**Если ACCEPT:** показать PROP-N пользователю через AskUserQuestion. Для каждого PROP-N — принять или отклонить. Отклонённые PROP-N переместить из секции "Предложения" в секцию "Отвергнутые предложения" (с указанием причины). Перейти к шагу 8.
...
7. После финального ACCEPT: показать PROP-N пользователю через AskUserQuestion (принять/отклонить). Отклонённые → "Отвергнутые предложения" (с причиной)
