...
**5.3. mode=system** (последний — системные тесты, кросс-зависимости, BLOCK-N по 1-2 TASK-N):
...
**При ACCEPT:** обработать PROP-N (оркестратор, НЕ агент):
   ...
   - **ВАЖНО: обрабатывать PROP по одному через Edit, НЕ заменять секцию «Предложения» целиком.**
     ...
     - Принятый PROP: (a) применить изменения к содержимому документа, (b) удалить строку PROP из секции «Предложения»
...
- [ ] mode=system: системные задачи, кросс-зависимости, BLOCK-N (1-2 TASK-N каждый) сгенерированы
...
5. Волна 1 — plandev-agent:
   ...
   5.3. mode=system: кросс-зависимости, BLOCK-N по 1-2 TASK-N (7 блоков, 3 волны)
