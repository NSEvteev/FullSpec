---
description: Стандарт analysis/ — полное описание аналитического контура SDD v2: философия двух контуров, 4 уровня, воркфлоу, 8 статусов, каскады, обновление docs/, Clarify, именование, запреты.
...
6. **Когда разработка завершена (все TASK-N готовы к ревью):**
...
| **DONE** | per-document | Реализация конкретного документа завершена, артефакты обновлены. Каскад снизу вверх (§ 6.6). **Финальный**, когда все документы цепочки — DONE |
...
| RUNNING | REVIEW | Tree-level — все задачи Plan Dev завершены, пользователь подтвердил (§ 6.5) |
| RUNNING | CONFLICT | Tree-level — обратная связь от кода (§ 6.3) |
| RUNNING | ROLLING_BACK | Tree-level — пользователь отменяет |
| REVIEW | DONE | review.md RESOLVED (вердикт READY). Per-document, bottom-up каскад (§ 6.6) |
| REVIEW | CONFLICT | Tree-level — review.md P1 (вердикт CONFLICT) |
| REVIEW | ROLLING_BACK | Tree-level — пользователь отменяет |
...
Design → DONE (триггер: Plan Tests → DONE, каскад § 6.6 шаг 3) обновляет AS IS в docs/: Planned Changes переносятся в основной контент, затем перемещаются в Changelog. Это первый уровень каскада, где обновляются docs/.
