...
**chain_status.py:** **T7** — `transition(to="DONE", document="plan-dev")`. Автоматический bottom-up каскад: plan-dev → plan-test → design → discussion. Возвращает `side_effects` для обновления docs/ (§ 7.3).
...
> **chain_status.py:** `transition()` возвращает `result.side_effects` — список конкретных действий для LLM (какие файлы docs/ создать, обновить, удалить). Модуль **не выполняет** обновление docs/ — это задача LLM по инструкциям create-*/modify-*. После обновления docs/ — вызвать `check_cross_chain()` для проверки влияния на другие цепочки (§ 7.2).
