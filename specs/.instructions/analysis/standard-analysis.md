---
description: Стандарт analysis/ — полное описание аналитического контура SDD v2: философия двух контуров, 4 уровня, воркфлоу, статусы, каскады, обновление docs/, Clarify, именование, запреты.
...
Полное описание аналитического контура Specification-Driven Development v2: философия двух контуров (analysis/ + docs/), четыре уровня (Discussion → Design → Plan Tests → Plan Dev), 7-шаговый воркфлоу объекта, 7 статусов, каскады, правила обновления docs/ при переходах, Clarify-паттерн, именование, запреты. Все per-object стандарты ссылаются на этот документ как SSOT.
...
- [4. Воркфлоу](#4-воркфлоу)
  ...
  - [4.2 Границы analysis/ ↔ docs/ ↔ проект](#42-границы-analysis-docs-проект)
...
- [7. Обновление docs/ при переходах](#7-обновление-docs-при-переходах)
...
**analysis/** — строительные леса. Группирует документы по изменению (цепочке). После завершения (DONE) остаётся как история решений — WHY за результатом в docs/.

**docs/** — доставочный формат. Разработчик читает только docs/ для понимания системы. Документы docs/ обновляются при переходах статусов в analysis/ ([§ 7](#7-обновление-docs-при-переходах)).
...
- analysis/ **читает** docs/ (при Scan) и **пишет в** docs/ (при переходах статусов)
- docs/ **не ссылается** на analysis/ — ни одна секция docs/ не содержит прямых ссылок на analysis/
...
graph TD
    ...
    DOCS["<b>docs/</b><br/>Живые документы"]
...
Поток: Дискуссия → Дизайн → План тестов → План разработки → GitHub Issues → Разработка.
...
| **План тестов** | `plan-test.md` | Как проверяем решение? | Per-service разделы: тестовые сценарии (e2e, integration, unit), acceptance criteria → тесты, тестовые данные | Реализацию тестов, задачи | [standard-plan-test.md](plan-test/standard-plan-test.md) |
...
|--------------------|---------------|------------|
| **Scan** | Impact (Quick Scan) + Design (Deep Scan) | Читает docs/: README, overview, {svc}.md, .technologies/. Unified Scan из 5 источников: Discussion + `docs/README.md` + `docs/.system/overview.md` + `docs/{svc}.md` + `docs/.technologies/` |
...
## Резюме
## SVC-N: {Сервис}
### 1. Назначение              ← delta к docs/{svc}.md § 1
### 2. API контракты           ← delta к docs/{svc}.md § 2  (ADDED/MODIFIED/REMOVED)
### 3. Data Model              ← delta к docs/{svc}.md § 3  (ADDED/MODIFIED/REMOVED)
### 4. Потоки                  ← delta к docs/{svc}.md § 4  (новые/изменённые сценарии)
### 5. Code Map                ← delta к docs/{svc}.md § 5  (пакеты, точки входа)
### 6. Зависимости             ← delta к docs/{svc}.md § 6
### 7. Доменная модель         ← delta к docs/{svc}.md § 7  (ADDED/MODIFIED/REMOVED)
### 8. Границы автономии LLM   ← delta к docs/{svc}.md § 8  (Свободно/Флаг/CONFLICT)
...
| 2 | **CLARIFY** | LLM читает входные данные (parent-документ, docs/ и т.д.), формулирует выводы и предлагает их пользователю через AskUserQuestion. Пользователь подтверждает или корректирует. При `--auto-clarify` — LLM пропускает вопросы, ставит маркеры `[ТРЕБУЕТ УТОЧНЕНИЯ]` |
...
| 2 | `docs/README.md` | Список сервисов, технологии | Обзор ландшафта |
| 3 | `docs/.system/overview.md` | Архитектура, data flows, Planned Changes | Текущее состояние + запланированные изменения |
| 4 | `docs/{svc}.md` | API, Data Model, Code Map, зависимости | Детали затронутых сервисов |
| 5 | `docs/.technologies/` | Per-tech стандарты кодирования | Технологические конвенции |

Design **предлагает и решает** (роли объединены): сканирует docs/, выявляет затронутые сервисы, распределяет ответственности, детализирует решения по реализации.
...
2. **`docs/.system/testing.md`** — текущая стратегия тестирования
3. **`docs/{svc}.md`** — API контракты для формирования тест-сценариев
...
5. **`docs/{svc}.md`** — текущее AS IS (для регрессионных тестов)
...
2. **Зависимости в docs/:** Каждый сервис указывает зависимости от shared/ в секции "Зависимости" своего `docs/{svc}.md`

3. **Зависимости задач:** Задача "создать схему в shared/" (из Plan Dev провайдера) блокирует задачи "реализовать обработчик" (из Plan Dev потребителей). Зависимость через `**Зависит от:** #N` в GitHub Issues
...
2. Design: DRAFT → [Unified Scan + итерации] → WAITING. **При переходе в WAITING** (одновременно): Planned Changes добавляются в docs/ ([§ 7.1](#71-обновление-при-планировании-to-waiting)), заглушки `{svc}.md` создаются для новых сервисов
...
### 4.2 Границы analysis/ ↔ docs/ ↔ проект

```
analysis/                     │  docs/                        │  Проект
                              │                               │
...
───── процесс ────────────────│───── результат ───────────────│──── код ─────
                              │                               │
analysis/ говорит ЗАЧЕМ и КАК │  docs/ говорит ЧТО ЕСТЬ      │  Код делает
```
...
1. Читает frontmatter всех 4 документов цепочки (поля `status`)
...
| WAITING | RUNNING | Tree-level — все 4 документа цепочки в WAITING, пользователь подтвердил |
...
| CONFLICT | WAITING | Per-document (top-down) — каждый документ ревьюится и переводится в WAITING последовательно. После того как ВСЕ 4 документа цепочки в WAITING — каскад RUNNING (§ 6.2, tree-level) |
...
Критерий масштаба — границы автономии из Code Map (`docs/{svc}.md` → секция "Границы автономии LLM"):
...
4. **Для DONE-документов:** LLM обновляет документ **и** артефакты (docs/, Issues)
5. **Незатронутые документы:** LLM отображает содержимое. Пользователь подтверждает, что документ остаётся верным → **WAITING** (без изменений контента)
6. **Затронутые документы:** Пользователь ревьюит каждый изменённый документ (из шагов 1–4) → каждый → **WAITING**
...
graph LR
    ...
    TRIGGER{"План разработки<br/>→ DONE?"}
...
|-------------|---------|-----------|----------------|
| 1 | Все задачи Plan Dev выполнены | Plan Dev → DONE | Разработчик / LLM-агент |
| 2 | Plan Dev → DONE | Plan Tests → DONE | Автоматически |
| 3 | Plan Tests → DONE | Design → DONE + обновление AS IS в docs/ ([§ 7.3](#73-обновление-при-реализации-to-done)) | Автоматически |
| 4 | Design → DONE | Discussion → DONE | Автоматически |

**Обновление docs/ при каскаде DONE:** Именно Design → DONE (шаг 3) является триггером переноса Planned Changes → AS IS в docs/. Design → DONE наступает **после** Plan Tests → DONE (шаг 2), т.е. все тесты уже пройдены.

**Кросс-цепочечная обратная связь:** при обновлении docs/ (через каскад DONE или разрешение CONFLICT) проверяются все другие цепочки — механизм и таблица реакций в [§ 7.2](#72-конфликт-исполнения-conflict).
...
| **Design** | Откат изменений в docs/: удаление Planned Changes из `{svc}.md` § 9, `overview.md` § 8. Удаление заглушек новых сервисов (если уникальны). Удаление per-tech стандартов + rule + строки реестра (если технология введена этим Design). Удаление меток `svc:{svc}` (если сервис создан этим Design) |
| **Plan Tests** | Откат изменений в `docs/.system/testing.md` |
...
## 7. Обновление docs/ при переходах

Документы docs/ — живое описание системы. Не имеют статусов — обновляются при планировании (§ 7.1), реализации (§ 7.3) и откате (§ 7.5). Обновление происходит **только при переходах статусов** в analysis/, не при обычных операциях внутри статуса.

| Документ docs/ | При WAITING | При DONE | При REJECTED |
|----------------|:-----------:|:--------:|:------------:|
| `docs/README.md` | R | — | — |
| `docs/.system/overview.md` | R+W | W | DEL |
| `docs/.system/conventions.md` | W? | W? | DEL? |
| `docs/.system/infrastructure.md` | W? | W? | DEL? |
| `docs/.system/testing.md` | — | W? | — |
| `docs/{svc}.md` (существующие) | R+W | R+W | DEL |
| `docs/{svc}.md` (новые) | CREATE | R+W | DEL? |
| `docs/.technologies/standard-{tech}.md` | R / CREATE | — | DEL? |
...
При переходе Design в WAITING создаются **Planned Changes** — структурированные дельты (ADDED/MODIFIED/REMOVED) в docs/, по одному блоку на каждую активную цепочку. LLM при чтении docs/ **обязан** учитывать Planned Changes. Удаляются при каскаде DONE (§ 7.3) или REJECTED (§ 7.5).

#### Design → WAITING

| Условие | Файл docs/ | Действие | Формат |
|---------|-----------|----------|--------|
...
| Архитектурные изменения | `.system/overview.md` § 8 | Добавить Planned Changes | [standard-overview.md](../docs/overview/standard-overview.md) |
...
При разрешении CONFLICT (§ 6.4) DONE-документы обновляются вместе с артефактами — docs/ изменяется на месте. Это затрагивает **другие цепочки**, ссылающиеся на изменённые файлы.

Реакция зависит от текущего статуса другой цепочки:

| Статус цепочки | Реакция |
|---|---|
| **DRAFT** | Затронутые документы перегенерируются с учётом нового docs/ |
...
**LLM автоматически** (без дополнительной команды) после каждого обновления docs/ проверяет Planned Changes всех активных цепочек — определяет затронутые файлы. Проверка происходит **до завершения** текущего каскада DONE/CONFLICT.
...
Design → DONE (триггер: Plan Tests → DONE, каскад § 6.5 шаг 3) обновляет AS IS в docs/: Planned Changes переносятся в основной контент, затем перемещаются в Changelog. Это первый уровень каскада, где обновляются docs/.
...
**Проблема:** Цепочка А в работе, цепочка Б стартует. Б не видит планируемых изменений от А — docs/ ещё не обновлены.

**Механизм:** Planned Changes в docs/ ([§ 7.1](#71-обновление-при-планировании-to-waiting)). Каждый блок Planned Changes содержит дельты ADDED/MODIFIED/REMOVED с данными из Design — другие цепочки видят запланированные изменения напрямую в docs/, без перехода в Design.

**Конфликт Planned Changes:** Если две цепочки добавляют Planned Changes в один и тот же `{svc}.md` — каждый блок маркируется идентификатором цепочки (`chain: NNNN`). LLM при чтении обязан учитывать оба блока. При реализации — CONFLICT не возникает автоматически; ответственность за непротиворечивость — на Design каждой цепочки.

### 7.5 Обновление при откате (ROLLING_BACK → REJECTED)

При откате (§ 6.6–6.7) все артефакты, созданные или изменённые отвергнутой цепочкой, должны быть приведены к состоянию "как будто этой цепочки не было".
...
| `.system/conventions.md` | Откат записей (если были) |
| `.system/infrastructure.md` | Откат записей (если были) |
...
**Запрет прямого редактирования docs/:** Документы docs/ обновляются **только** через переходы статусов в analysis/ ([§ 7](#7-обновление-docs-при-переходах)). Прямое редактирование docs/ без цепочки в analysis/ запрещено — нарушает SSOT.

**Исключение для docs/:** Исправление опечаток, форматирования и мелких неточностей допускается без analysis/. Критерий: изменение **не меняет поведение системы** — не затрагивает API контракты, data model, схему интеграций, описание архитектурных решений.
...
| 3 | Два контура | **analysis/** (процесс: обсуждение, проектирование) + **docs/** (результат: живое AS IS). Разделение зон ответственности |
...
| 6 | Unified Scan | Объединённый Quick + Deep Scan. 5 источников: Discussion + docs/README.md + overview.md + {svc}.md + .technologies/ |
...
| 13 | Модель статусов | **7 статусов:** DRAFT, WAITING, RUNNING, DONE, CONFLICT, ROLLING_BACK, REJECTED |
| 14 | Каскад DONE | **Per-document, bottom-up.** Plan Dev → Plan Tests → Design → Discussion |
| 15 | Каскад CONFLICT | **Tree-level.** Все 4 документа цепочки → CONFLICT (включая DONE) |
| 16 | Каскад ROLLING_BACK/REJECTED | **Tree-level.** Все документы (включая Discussion) → ROLLING_BACK → REJECTED |
...
| 20 | Design v2 = Impact + Design + ADR | 5 фаз: Scan → Decide → Detail → Contract → Test Scenarios |
...
| 24 | Параллельные цепочки | **Planned Changes в docs/**. Кросс-цепочечная осведомлённость без маркеров конфликта |
| 25 | Dependency Barrier | При зависимости маркера от неразрешённого → LLM прекращает генерацию |
| 26 | Системные тест-сценарии | **Секция STS-N в Design**. При DONE → `docs/.system/testing.md` |
...
| 29 | Кросс-цепочечная обратная связь | При обновлении docs/ проверить все другие цепочки |
| 30 | Очистка REJECTED | По команде пользователя LLM предлагает список |
| 31 | Запрет прямого редактирования docs/ | docs/ обновляется только через analysis/. Исключение: опечатки и форматирование |
| 32 | Агенты | Design → design-agent (Unified Scan + генерация, последовательно). ADR Builder/Reconciler не нужны |
