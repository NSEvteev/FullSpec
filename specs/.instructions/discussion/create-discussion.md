...
- [Шаги](#шаги)
  ...
  - [Шаг 8.5: Ревью агентом (опционально)](#шаг-85-ревью-агентом-опционально)
...
**SSOT:** [standard-discussion.md § 6](./standard-discussion.md#6-clarify), [Стандарт SDD § 10](../standard-specs.md#10-clarify-и-блокирующие-правила)
...
**Dependency Barrier:** Если генерация следующей секции зависит от неразрешённого маркера — остановить генерацию контента и перейти к перечислению оставшихся секций ([Стандарт SDD § 10](../standard-specs.md#dependency-barrier)).
...
3. Для каждого PROP — AskUserQuestion: принять, отклонить или изменить
4. Если принят — LLM вносит изменения в затронутые секции (F-N, US-N, REQ-N и т.д.), удаляет PROP
5. Если отклонён — удаляет PROP
6. Когда все PROP обработаны — заменить содержимое секции на: _Все предложения проработаны._
...
### Шаг 8.5: Ревью агентом (опционально)

**Агент:** [discussion-reviewer](/.claude/agents/discussion-reviewer/AGENT.md)

После валидации — AskUserQuestion: "Запустить агент discussion-reviewer для анализа полноты?"

| Ответ | Действие |
|-------|----------|
| Да | Запустить агента `discussion-reviewer` с путём к документу. Агент запишет PROP-N в секцию "Предложения". После возврата агента — обработать PROP-N (шаг 6, подраздел "Генерация и проработка предложений") → вернуться к шагу 8 |
...
⏭️ **Следующий шаг:** Создать Impact Analysis
...
- [ ] Ревью агентом проведено (опционально, шаг 8.5)
...
|-------|------------|-----|
| [discussion-reviewer](/.claude/agents/discussion-reviewer/AGENT.md) | Ревью дискуссии на полноту, генерация PROP-N | Шаг 8.5, Шаг 9 |
