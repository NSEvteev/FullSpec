...
**Оптимизация создания (2 раунда вместо 25 вызовов):**
1. **Раунд 1:** Все 13 TaskCreate параллельно в одном сообщении (без blockedBy — TaskCreate не поддерживает)
2. **Раунд 2:** Все 12 TaskUpdate (addBlockedBy) параллельно — ID предсказуемы (последовательные числа от первого созданного)
