...
| **Импакт** | НА ЧТО ВЛИЯЕТ | `specs/impact/` | Какие сервисы предположительно затронуты? Какие риски? Quick Scan `specs/architecture/` | Дискуссия → Дизайн |
...
| **НА ЧТО ВЛИЯЕТ** | `impact/` | Какие сервисы предположительно затронуты? | Варианты затронутых сервисов, shared-компоненты, планы создания новых сервисов, риски, зависимости. Quick Scan: `specs/architecture/` | Окончательное распределение ответственностей (→ Design РЕШАЕТ) |
...
| **АРХИТЕКТУРА** | `specs/architecture/` | Как устроена система сейчас? | Живое AS IS: system/, services/ (включая Code Map), domains/. Planned Changes | Исторические решения (в ADR) |
...
3. Design: DRAFT → [итерации] → WAITING → Planned Changes добавляются в `specs/architecture/`, `services/{svc}.md` создаётся (stub) для новых сервисов
...
**Greenfield:** Если сервис новый, stub `specs/architecture/services/X.md` создаётся при Design → WAITING (с Резюме и Planned Changes). При ADR → DONE stub заполняется полным содержанием — вся дельта = ADDED. Code Map создаётся при первом ADR → DONE.
...
**Критерий обнаружения:** При работе на уровне N, LLM проверяет каждый факт/вывод: попадает ли он в **зону ответственности** вышестоящего уровня ([§ 3](#3-зоны-ответственности)) и **отсутствует** ли в документе этого уровня? Если оба условия выполнены — это upward feedback.

Ориентир — колонка **"Вопрос"** из таблицы зон: если обнаруженная информация отвечает на вопрос вышестоящего уровня и этот ответ там не зафиксирован — дополнить.
...
| ADR → DONE | все планы тестов DONE | обновление `specs/architecture/services/{svc}.md` (дельта → AS IS), обновление Planned Changes |
| Design → DONE | все ADR DONE | обновление `specs/architecture/system/`, `domains/`, `tests/system/`. Planned Changes → Changelog в `services/{svc}.md`, `system/`, `domains/` |
...
**Кросс-цепочечная обратная связь:** при обновлении `specs/architecture/` (через каскад DONE или разрешение CONFLICT) проверяются все другие цепочки — механизм и таблица реакций в [§ 9.2](#92-конфликт-исполнения-conflict).
...
| **Design** | Откат изменений в `specs/architecture/system/`, `specs/architecture/domains/`, `tests/system/`. Удаление Planned Changes из `specs/architecture/` |
...
При разрешении CONFLICT (§ 8.4) DONE-документы обновляются вместе с артефактами — `specs/architecture/` изменяется на месте. Это затрагивает **другие цепочки**, ссылающиеся на изменённые файлы.
...
**Механизм:** Planned Changes в `specs/architecture/` (§ 9.1). LLM при чтении AS IS **обязан** учитывать Planned Changes. При необходимости LLM переходит по ссылке на Design и читает дочерние ADR (через frontmatter `children`) для получения конкретных дельт.
