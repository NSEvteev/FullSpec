...
|--------|-------------|--------|----------|-------------|------|
| **Архитектура** | `specs/architecture/` | Как устроена система сейчас? | Двухслойная модель: AS IS (факт) + Planned Changes (дельты ADDED/MODIFIED/REMOVED по каждому Design). system/, services/, domains/ | Исторические решения (в ADR) | [standard-service.md](living-docs/service/standard-service.md) |
...
| **Design** | Откат изменений в `specs/architecture/system/`, `specs/architecture/domains/`, `tests/system/`. Удаление Planned Changes ([§ 7.1](#71-обновление-при-планировании-to-waiting)). Удаление per-tech стандартов + rule + строки реестра (если технология введена этим Design) |
| **ADR** | Откат изменений в `specs/architecture/services/{svc}.md` (по дельта-блокам: ADDED удаляются, MODIFIED возвращаются к предыдущему состоянию, REMOVED восстанавливаются). Удаление Planned Changes |
...
| **Архитектура (сервисная)** | `specs/architecture/services/{svc}.md` | Двухслойная: AS IS (API, data model, Code Map, границы LLM) + Planned Changes (дельты по каждому Design) |
...
| 30 | Параллельные дискуссии | **Planned Changes в architecture/** — дельты ADDED/MODIFIED/REMOVED по каждому Design. Кросс-дискуссионная осведомлённость без маркеров конфликта |
...
| 36 | Технологические стандарты | Per-tech стандарты (`standard-{tech}.md` + `validation-{tech}.md`) размещаются в `specs/technologies/`, мета-стандарт в [standard-technology.md](/specs/.instructions/technologies/standard-technology.md). **Двухфазная модель:** Design → WAITING создаёт заглушку per-tech стандарта (декларация выбора технологии), ADR → DONE заполняет конвенциями кодирования |
