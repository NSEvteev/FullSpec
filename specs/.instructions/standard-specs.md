...
| **Дизайн** | `specs/design/` | Как распределяем ответственности? (РЕШАЕТ) | Секции по сервисам, блоки взаимодействия, контракты API, системные тест-сценарии | Детали реализации конкретного сервиса | Импакт → ADR(ы) | [standard-design.md](design/standard-design.md) |
...
**Greenfield:** Если сервис новый, заглушка `specs/architecture/services/X.md` создаётся при Design → WAITING ([§ 7.1](#71-обновление-при-планировании-to-waiting)). При ADR → DONE заглушка заполняется полным содержанием — вся дельта = ADDED. Code Map создаётся при первом ADR → DONE.
...
3. Design: DRAFT → [итерации] → WAITING → Planned Changes добавляются в `specs/architecture/` ([§ 7.1](#71-обновление-при-планировании-to-waiting)), `services/{svc}.md` создаётся (заглушка) для новых сервисов
...
| **Design** | Откат изменений в `specs/architecture/system/`, `specs/architecture/domains/`, `tests/system/`. Удаление Planned Changes ([§ 7.1](#71-обновление-при-планировании-to-waiting)). Удаление заглушек per-tech стандартов + rule + строки реестра (если технология введена этим Design) |
...
| **Технологический реестр** | `specs/technologies/README.md` | реестр технологий проекта (технология, версия, сервисы, ссылка на per-tech стандарт) |

### 7.1 Обновление при планировании (to WAITING)

При переходе в WAITING создаются **Planned Changes** — навигационные указатели в `specs/architecture/` на запланированные изменения. LLM при чтении AS IS **обязан** учитывать Planned Changes. Удаляются при каскаде DONE (§ 7.3) или REJECTED.

| Уровень | Что происходит | Подробности |
|---------|---------------|-------------|
| **Design → WAITING** | Planned Changes в `services/{svc}.md`, `system/`, `domains/`. Заглушка для новых сервисов. Заглушки per-tech стандартов для новых технологий + обновление реестра. Создание ADR (1:N) | [standard-design.md § 4](design/standard-design.md#4-переходы-статусов) |
...
| **Технологические стандарты** | ADR → DONE (per-tech стандарт заполняется конвенциями кодирования). Design → DONE (нет изменений — заглушка уже создана при WAITING) |
...
| 2. Импакт | [standard-impact.md](./impact/standard-impact.md) | [impact/](./impact/) |
| 3. Дизайн | [standard-design.md](./design/standard-design.md) | [design/](./design/) |
...
| Технологии | [standard-technology.md](./technologies/standard-technology.md) | [technologies/](./technologies/) |
...
| 36 | Технологические стандарты | **`specs/.instructions/technologies/`** — пара standard + validation, мета-стандарт [standard-technology.md](/specs/.instructions/technologies/standard-technology.md). **Двухфазная модель:** Design → WAITING создаёт заглушку per-tech стандарта (декларация выбора технологии), ADR → DONE заполняет конвенциями кодирования |
