...
```mermaid
graph TD
    PD["<b>План разработки → DONE</b>

    все задачи выполнены"]

    TD["<b>План тестов → DONE</b>

    все планы разработки DONE
...
| ADR → DONE | все планы тестов DONE | обновление `architecture/services/{svc}.md`, удаление Planned Changes |
...
ADR auth отклоняется:
  ...
  │         План разработки auth (RUNNING) → ROLLING_BACK → REJECTED (закрытие Issues)
  ├── сиблинги:
  ...
  │         План разработки gateway (DONE) → ROLLING_BACK → REJECTED (закрытие Issues)
...
ADR (RUNNING) → ROLLING_BACK → REJECTED
  ...
  │         План разработки (RUNNING) → ROLLING_BACK → REJECTED
  └── вверх: Design → DRAFT
...
| **Флаг** (контракты между пакетами) | План разработки / План тестов | Рабочие правки — LLM автономно обновляет документы, продолжает работу и выводит в чат краткое резюме изменений (не дожидаясь ответа). Статус не меняется. Фаза 2 не нужна |
...
Результат:
  ADR auth, План тестов auth, План разработки auth → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  3. План разработки auth пересматривается
...
Фаза 1 — Обнаружение (↑):
  ...
  Design затронут? → Да (блоки взаимодействия изменились —
    ...
    → Design → CONFLICT → ВСЕ дети:
      ...
      → ВСЕ их дети (План тестов, План разработки) → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  4. План разработки для каждого пересматриваются
...
Фаза 1 — Обнаружение (↑):
  ...
  Impact затронут? → Да ("3 сервиса" → "все сервисы проекта")
    → Impact → CONFLICT → Design → CONFLICT
      → ВСЕ ADR, План тестов, План разработки → CONFLICT
...
Фаза 2 — Разрешение (↓):
  ...
  5. План разработки для каждого пересматриваются
