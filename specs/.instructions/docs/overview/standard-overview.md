...
**Синхронизация с {svc}.md.** Колонки «Зона ответственности» и «Ключевые API» в Карте сервисов — сокращения из соответствующего `{svc}.md`. При изменении `{svc}.md` (новый endpoint, изменение зоны) — обновить overview.md. После создания или изменения overview.md — запустить валидацию: см. [validation-overview.md](./validation-overview.md).
...
|--------|---------------------|----------------|-------------|
| {svc} | {что делает} | {таблицы/коллекции} | {2-4 endpoints} |
