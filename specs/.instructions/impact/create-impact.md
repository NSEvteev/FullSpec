...
- [Шаги](#шаги)
  ...
  - [Шаг 9.5: Ревью агентом (опционально)](#шаг-95-ревью-агентом-опционально)
...
На основе результатов Quick Scan (шаг 2), Discussion (шаг 1) и Clarify (шаг 6) заполнить документ **по сервисам**:

1. **📋 Резюме** — scope влияния, характер изменений, ключевые risk areas (контент обязателен)
2. **Секции SVC-N** (по одной на каждый затронутый сервис, мин. 1):
   - Заголовок: `## SVC-N: {сервис}` с метаданными (Тип влияния, Уверенность) и описанием флоу (1–2 абзаца: триггер, поток, результат, связи)
   - `### 📦 Компоненты` — таблица CMP-N или заглушка: _Компоненты не идентифицированы._
   - `### 💾 Данные и хранение` — таблица DATA-N или заглушка: _Изменений в данных нет._
   - `### 🔌 API` — таблица API-N или заглушка: _Изменений в API нет._
3. **🔗 Зависимости** — таблица DEP-N или заглушка: _Зависимостей нет._
4. **⚠️ Риски** — таблица RISK-N (минимум 1 элемент)

**Порядок секций SVC:** от Основного к Косвенному. Нумерация CMP-N, DATA-N, API-N — сквозная по всему документу.
...
### Шаг 9.5: Ревью агентом (опционально)

**Агент:** [impact-reviewer](/.claude/agents/impact-reviewer/AGENT.md)

После валидации — AskUserQuestion: "Запустить агент impact-reviewer для анализа полноты?"

| Ответ | Действие |
|-------|----------|
| Да | Запустить агента → обработать PROP-N → вернуться к шагу 9 |
| Нет | Перейти к шагу 10 |

**Обработка PROP-N после возврата агента:**

1. Для каждого PROP-N — AskUserQuestion: принять, отклонить или изменить
2. **Обычные PROP** (без `↑`): если принят — применить к Impact, если отклонён — удалить
3. **Upward feedback PROP** (с `↑ Discussion:`): если принят — обновить parent Discussion ([standard-specs.md § 3.6](../standard-specs.md#36-upward-feedback)):
   - Приостановить работу с Impact
   - Внести изменения в соответствующую секцию Discussion (Фичи, Требования, Критерии)
   - AskUserQuestion: подтвердить изменения в Discussion
   - Статус Discussion остаётся WAITING
   - Возобновить работу с Impact
4. Когда все PROP обработаны → вернуться к шагу 9
...
| Запустить ревью агентом | Запустить агента `impact-reviewer` → обработать PROP-N (шаг 9.5) → вернуться к шагу 9 |
...
| Ответ | Действие |
|-------|----------|
| Да | Вызвать `/design-create` с путём к текущему Impact |
...
- [ ] Ревью агентом проведено (опционально, шаг 9.5)
...
7. Разделы: Резюме, SVC-1..SVC-4 (с подсекциями), Зависимости (DEP-1..DEP-2), Риски (RISK-1..RISK-2)
...
6. Clarify: LLM предлагает "cache-service (основной, единственный затронутый)"
   → Пользователь: "Верно"
7. Разделы: Резюме, SVC-1 (CMP-1..CMP-2, заглушки данных и API), Зависимости (заглушка), Риски (RISK-1)
   → Проверка маркеров: 0 → OK
8-9. README + Валидация
10. Ревью: "Да, всё корректно" → DRAFT → WAITING
11. Отчёт
...
|-------|------------|-----|
| [impact-reviewer](/.claude/agents/impact-reviewer/AGENT.md) | Ревью импакт-анализа на полноту, генерация PROP-N | Шаг 9.5, Шаг 10 |
