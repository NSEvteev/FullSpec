...
6. СОЗДАНИЕ PR
   └─ /review-branch (локальное ревью до создания PR)
   └─ git push -u origin {branch}
   └─ gh pr create --title "..." --body "Closes #42, #43, #44"
   └─ → standard-pull-request.md

7. CODE REVIEW
   └─ /review-pr {номер} (агент пишет комментарии)
...
- **Merge в main ≠ деплой.** Код на production попадает ТОЛЬКО через Release — см. [standard-release-workflow.md](./releases/standard-release-workflow.md)
...
**SSOT:** [standard-release-workflow.md](./releases/standard-release-workflow.md), [standard-release.md](./releases/standard-release.md)
