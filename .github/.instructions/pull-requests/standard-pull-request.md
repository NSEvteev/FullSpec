...
1. Локальное ревью (`/review-branch`) — до создания PR
2. Комментарии к PR (`/review-pr {номер}`) — после создания
3. Исправление замечаний → push
4. Merge: `gh pr merge {номер} --squash`
