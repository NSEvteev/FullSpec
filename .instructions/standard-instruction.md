...
- [4. Инструкции типа modify](#4-инструкции-типа-modify)
  ...
  - [Секция "Переименование"](#секция-переименование)
  - [Секция "Перемещение"](#секция-перемещение)
  - [Секция "Удаление"](#секция-удаление)
...
**Секция "Чек-лист":**
- контрольный список в формате `- [ ]`
- повторяет шаги в сжатом виде

**Секция "Примеры":**
- конкретные примеры создания с реальными путями
- формат: `### {Сценарий}` с bash-командами

**Секция "Скрипты":**
- таблица `| Скрипт | Назначение | Инструкция |`
- блок с примерами использования
...
**Инструкция типа modify** — описывает процессы изменения объекта: переименование, перемещение, удаление.
...
4. Раздел "Содержание"
   ...
   - Секция "Переименование" (шаги)
   - Секция "Перемещение" (шаги)
   - Секция "Удаление" (шаги)
...
| Тип | Описание | Пример |
|-----|----------|--------|
| Переименование | Изменение имени | `utils/` → `helpers/` |
| Перемещение | Изменение родителя | `src/utils/` → `shared/utils/` |
| Удаление | Полное удаление | `legacy/` → ∅ |
```

**Секции по типам (Переименование, Перемещение, Удаление):**
...
| Тип | Описание | Пример |
|-----|----------|--------|
| Переименование | {описание} | {пример} |
| Перемещение | {описание} | {пример} |
| Удаление | {описание} | {пример} |
...
Секции "Скрипты" и "Скиллы" присутствуют во всех трёх типах и имеют одинаковый формат:
...
Если скриптов или скиллов нет — `*Нет скриптов.*` / `*Нет скиллов.*`
