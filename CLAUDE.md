...
## Запрет ручного создания

> **КРИТИЧЕСКОЕ ПРАВИЛО:** Нельзя создавать инструкции и скиллы вручную!

| Объект | Правильно | Неправильно |
|--------|-----------|-------------|
| Инструкция | `/instruction-create путь` | `Write` файла напрямую |
| Скилл | `/skill-create имя` | `mkdir` + `Write` SKILL.md |

**Почему:**
- Скиллы выполняют полный воркфлоу (ревью, связи, статусы)
- Ручное создание пропускает обязательные шаги
- Нарушение приводит к несинхронизированным документам

**При нарушении:** Остановиться и выполнить пропущенные шаги через соответствующий скилл.
