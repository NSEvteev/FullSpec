...
**Любое изменение системы начинается с `/chain`** — оркестратор создаёт TaskList с полной последовательностью от идеи до релиза.

SSOT процесса: [standard-process.md](/specs/.instructions/standard-process.md)

Описывает три пути: Happy Path (12 задач TaskList), CONFLICT (обратная связь код → спецификации), альтернативные маршруты (Rollback, Hotfix, Doc-only).
